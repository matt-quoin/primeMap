...
function buildMap(countryData){
    ...
    var svg = divElement.append('svg').attr("x", 0).attr("y", 0).attr("width", '80%').attr("height", '73%').attr("class", "mapSVG"); var sidePanel = divElement.append("svg").attr("width", "20%").attr("height", "70%").attr("class", "sidePanel");
    var topPanel = reDraw();
    ...
    var collectedData = coagData(["internetUsers.csv", "perCapitaFoodSupply.csv", "undernourishment.csv", "poverty.csv", "gdpPerCap.csv"], data, 0, 
    function(dat){
...
function fillPC(){ //highlights primero countries in red
...
function updateMap(countryData, year){ //called whenever a country or button is clicked
    ...
    countries.selectAll("path")
        ...
        if (primeCountriesLower.indexOf(getKey(d.id,0, true)) > -1 ){ //only draws graph for primero countries
...
console.log("graphHeight: " + graphHeight);
  ...
  context.append("g")
    ...
    focus.append('text').attr("class", "graphTitleText").attr("x", graphWidth/2 - margin.left - margin.right).attr("y", selectButtonHeight + 4).style("font-size", "1.5em").style("font-family", "Helvetica Neue").style("font-weight", "bold").text(nameKey[0][dataSelect] + "-" + getKey(prevCountry.id,0, true));
...
var dataSelecter = d3.select(".dataSelecter");
    ...
    dataSelecter.selectAll('rect')
        .data(["perCapitaFoodSupply.csv", "undernourishment.csv", "poverty.csv"])
        .enter()
        .append('rect')
        .attr("class", "attributeButtons")
        .attr("x", function(d, i){return i * (graphWidth-margin.left-margin.right)/4 + margin.left})
        .attr("y", 5)
        .attr("width", (graphWidth - margin.left - margin.right)/5)
...
function drawLines(countryID){
    ...
    countries.selectAll("path").filter(function(d){
        ...
        console.log("presize: " + size);
        size = mainScale(size);
        console.log("size: " + size);
        center = [center, size];
            
            var gdp = parsed.findIndex(item => item.key == getKey(d.id,0, true).toUpperCase()); 
            var gdpScaledValue = 0;
                
            if(gdp != -1){ //if there is data on the country's gdp
            ...
            svg.append("line")
                ...
                .attr("x1", center[0][0]) //change to moveTo if you want selected country to have immigrants coming to it 
