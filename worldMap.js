...
function buildMap(countryData){
    ...
    var sidePanel = divElement.append("svg")
        .attr("width", "20%")
        .attr("height", "70%")
    ...
    var buttonWidth = parseInt(sidePanel.style("width")) - margin.right - margin.left;
        ...
        sidePanel.append('g').attr("class", "graph").selectAll('rect') //adds country select buttons
            ...
            .attr("x", (sideWidth - buttonWidth)/2)
...
function fillPC(){ //highlights primero countries in red after map updates
...
function drawBars(countryID, parsedData){ //draws bottom of page graph
    ...
    for(var y1 = 0; y1<parsedData.length; y1++){
        ...
        try{
            ...
            if(lowerString.toLowerCase() == countryName.toLowerCase()) { //if the country path selected matches the country name in the data
...
function drawLines(countryID){ //draws paths to countries
