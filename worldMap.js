...
function buildMap(countryData) {
  ...
  var collectedData = coagData(
    ["internetUsers.csv", "perCapitaFoodSupply.csv", "undernourishment.csv", "poverty.csv", "gdpPerCap.csv"], data, 0,
    function(dat) {
...
function updateMap(countryData, year) { //called whenever a country or button is clicked
  ...
  function drawBars(countryID, parsedData) { //draws bottom of page graph
    ...
    for(var y1 = 0; y1<parsedData.length; y1++) {
      ...
      try {
        if (typeof lowerString !== 'undefined' && lowerString.toLowerCase() == countryName.toLowerCase()) { //if the country path selected matches the country name in the data
